...
class Datacollector extends Controller {
    ...
    public function twinfieldData() {
        ...
        try {
            ...
            /*
              ...
              DB::table('syncsupplier')->where('frenns_id', $uniqueId)->where('type','customer')->delete();
            ...
            /*
              ...
              if(!empty($contacts)){
              ## Delete all customer from database
              DB::table('syncsupplier')->where('frenns_id', $uniqueId)->where('type','supplier')->delete();
	...
	public function quickbookData()
	{
		...
		$getCountdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();			
		$countDet 		=	count($getCountdetail);			
		if($countDet < 10)
		{				
			$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();					
			$this->getInnerquickbookData($getdetail,$accounting_system);
		}
		else
		{
			...
			{
				$offset	= $i * 10;					
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->offset($offset)->limit(10)->get();
				$this->getInnerquickbookData($getdetail,$accounting_system);
			}				
		}
	}
	
    public function getInnerquickbookData($getdetail,$accounting_system) 
	{     
        if (!empty($getdetail)) 
		{
			// echo "<pre>";
            ...
            foreach ($getdetail as $user) {
                ...
                $accountSystem		= $user->accounting_system;

                ########################### Vendor Section ################################################

                $url = "https://sandbox-quickbooks.api.intuit.com/v3/company/" . $realm . "/query?query=select%20*%20from%20Vendor";
                ...
                if (!empty($response)) {
                    ...
                    foreach ($response['QueryResponse']['Vendor'] as $key => $customer) {
						            $updateId = $accountSystem.'-'.$uniqueId . '-' . $customer['Id'];
                        ...
                        if ($customer['MetaData']['CreateTime'] > $lastCronTime || $lastCronTime == '') {
                            ...
                            $customerData[$key]['updateId'] = isset($customer['Id']) ? $updateId : '';
                            ...
                            $updateCustomerData[$key]['updateId'] = isset($customer['Id']) ? $updateId : '';
                ...
                $url = "https://sandbox-quickbooks.api.intuit.com/v3/company/" . $realm . "/query?query=Select%20*%20from%20Customer";
                $signature = Helper::qbOauthSignature($url, $user);
                ...
                $url = "https://sandbox-quickbooks.api.intuit.com/v3/company/" . $realm . "/query?query=select%20*%20from%20Invoice";
                $signature = Helper::qbOauthSignature($url, $user);
                $response = Helper::qbCurlRequest($url, $signature[3]);
                //echo "<pre>"; print_r($response);die;
                ...
                if (!empty($response)) {
                    ...
                    foreach ($response['QueryResponse']['Invoice'] as $key => $invoice) {
						            $updateIdQuery = $accountSystem.'-'.$invoice['Id'];
                        //echo "<prE>"; print_r($invoice); 
                        //echo "<br>".$invoice['MetaData']['CreateTime'] ." > ". $lastCronTime. "<br>";
                        //die('single invoice');
                        $address = '';
                        ...
                        if ($invoice['MetaData']['CreateTime'] > $lastCronTime || $lastCronTime == '') {
                            ...
                            $invoiceData[$key]['company_account_number'] = $uniqueId;
                            ...
                            $invoiceData[$key]['issue_date'] = isset($invoice['TxnDate']) ? $invoice['TxnDate'] : '';
                            ...
                            $invoiceData[$key]['vat_amount'] = '0.0';
                            $invoiceData[$key]['outstanding_amount'] = '0.0';
                            $invoiceData[$key]['paid'] = '';
                            $invoiceData[$key]['pay_date'] = '';
                            $invoiceData[$key]['updateId'] = isset($invoice['Id']) ? $updateIdQuery  : '';

                            #Save Quickbook Invoices 
                            if (!empty($invoiceData)) {
                                //echo "<prE>"; print_r($invoiceData[$key]); die;
                                //$this->apidetail->addInvoice($invoiceData);
                            ...
                            if (isset($invoice['Line'])) {
                                ...
                                foreach ($invoice['Line'] as $invoiceLine) {
                                    if (isset($invoiceLine['Id'])) {
                                        $quickbookItemId = $accountSystem.'-'.$invoice['Id'] . $invoiceLine['Id'];
                                        $invoiceItemData[$key . $itemKey]['frenns_id'] = $uniqueId;
                                        $invoiceItemData[$key . $itemKey]['invoice_number'] = isset($invoice['DocNumber']) ? $invoice['DocNumber'] : '';
                                        $invoiceItemData[$key . $itemKey]['line_number'] = isset($invoiceLine['Id']) ? $invoiceLine['Id'] : '';
                                        ...
                                        $invoiceItemData[$key . $itemKey]['qty'] = isset($invoiceLine['SalesItemLineDetail']['Qty']) ? $invoiceLine['SalesItemLineDetail']['Qty'] : '0';
                                        $invoiceItemData[$key . $itemKey]['rate'] = isset($invoiceLine['SalesItemLineDetail']['UnitPrice']) ? $invoiceLine['SalesItemLineDetail']['UnitPrice'] : '0';
                                        $invoiceItemData[$key . $itemKey]['amount_net'] = isset($invoiceLine['Amount']) ? $invoiceLine['Amount'] : '';
                                        $invoiceItemData[$key . $itemKey]['invoiceline_vat_amount'] = '0.0';
                                        $invoiceItemData[$key . $itemKey]['amount_total'] = isset($invoiceLine['Amount']) ? $invoiceLine['Amount'] : '';
                                        $invoiceItemData[$key . $itemKey]['invoice_id'] = isset($invoice['Id']) ? $invoice['Id'] : '';
                                        $invoiceItemData[$key . $itemKey]['updateId'] = $quickbookItemId;

                                        #Save Quickbook Invoices items 
                                        if (!empty($invoiceItemData[$key . $itemKey])) {
                                            //echo "<prE>"; print_r($invoiceItemData[$key . $itemKey]); die;
                            ...
                            $updateInvoiceData[$key]['company_account_number'] = $uniqueId;
                            ...
                            $updateInvoiceData[$key]['issue_date'] = isset($invoice['TxnDate']) ? $invoice['TxnDate'] : '';
                            ...
                            $updateInvoiceData[$key]['vat_amount'] = '';
                            $updateInvoiceData[$key]['outstanding_amount'] = '';
                            $updateInvoiceData[$key]['paid'] = '';
                            $updateInvoiceData[$key]['pay_date'] = '';
                            $updateInvoiceData[$key]['updateId'] = isset($invoice['Id']) ? $updateIdQuery : '';

                            #Update Quickbook Invoices 
                            if (!empty($updateInvoiceData)) {
                                //$this->apidetail->updateInvoices($updateInvoiceData, 'quickbookId');
                                DB::table('syncinvoice')->insert($updateInvoiceData[$key]);
                            }
                            ...
                            if (isset($invoice['Line'])) {
                                ...
                                foreach ($invoice['Line'] as $invoiceLine) {
                                    ...
                                    if (isset($invoiceLine['Id'])) {
                                        ...
                                        $UpdateInvoiceItemData[$key . $itemKey]['invoice_number'] = isset($invoice['DocNumber']) ? $invoice['DocNumber'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['line_number'] = isset($invoiceLine['Id']) ? $invoiceLine['Id'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['product_code'] = '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['description'] = isset($invoiceLine['description']) ? $invoiceLine['description'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['qty'] = isset($invoiceLine['SalesItemLineDetail']['Qty']) ? $invoiceLine['SalesItemLineDetail']['Qty'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['rate'] = isset($invoiceLine['SalesItemLineDetail']['UnitPrice']) ? $invoiceLine['SalesItemLineDetail']['UnitPrice'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['amount_net'] = isset($invoiceLine['Amount']) ? $invoiceLine['Amount'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['invoiceline_vat_amount'] = '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['amount_total'] = isset($invoiceLine['Amount']) ? $invoiceLine['Amount'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['invoice_id'] = isset($invoice['Id']) ? $invoice['Id'] : '';
                                        $UpdateInvoiceItemData[$key . $itemKey]['updateId'] = $quickbookItemId;
                                        $itemKey++;

                                        #Update Quickbook Invoices items 
                                        if (!empty($UpdateInvoiceItemData)) {
                                            //$this->apidetail->updateInvoicesItems($UpdateInvoiceItemData, 'quickbookItemId');
                ...
                $url = "https://sandbox-quickbooks.api.intuit.com/v3/company/" . $realm . "/reports/TransactionList?start_date=2015-06-01&end_date=2017-07-01&group_by=Customer";
                $signature = Helper::qbOauthSignature($url, $user);
                ...
                if (!empty($response)) {
                    ...
                    DB::table('syncledger_transaction')->where('frenns_id', $uniqueId)->delete();

                    #Delete ALl Nominal Ledger For current customer            
                    //$this->apidetail->deleteQuickbookNominalLedger($uniqueId);
                    DB::table('syncnominal')->where('frenns_id', $uniqueId)->delete();
                            ...
                            foreach ($value['Rows']['Row'] as $key2 => $value2) {
                                if (isset($value2['ColData'][8]['value']) && $value2['ColData'][8]['value'] != '') {
                                    ...
                                    if ($defaultAccountNo < 4000) {
                                        ...
                                        $transactionData[$key2]['updateId'] = $uniqueId;

                                        #Save Transaction For current customer
                                        if (!empty($transactionData)) {
                                            //echo "<pre>"; print_r($transactionData); die('----');
                                            //$this->apidetail->addLedgerTransaction($transactionData);
                                        ...
                                        $nominalTransactionData[$key2]['updateId'] = $uniqueId;

                                        #Save Nominal Transaction For current customer
                                        if (!empty($nominalTransactionData)) {
                                            //echo "<pre>"; print_r($nominalTransactionData[$key2]); die('======');
                                            //$this->apidetail->addNominalLedger($nominalTransactionData);
        ...
        die('Quickbook Process complete');
    }
    ...
    function clearbookData() {
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();					
				$this->getInnerClearBookData($getdetail,$accounting_system);
			}
			else
			{
				...
				{
					$offset	= $i * 10;					
					$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->where('usernumber', $userNumber)->offset($offset)->limit(10)->get();
					$this->getInnerClearBookData($getdetail,$accounting_system);
				}				
			}
        } 
	...
	public function getInnerClearBookData($getdetail,$accounting_system)
	{
		if (!empty($getdetail)) 
		{
			// echo "<pre>";
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                ...
                $accountSystem 	= $getdetail[$i]->accounting_system;

                // max time for invoice 
                $intype = '1';                
                $getLastUpdatedInvoice = DB::table('syncinvoice')->where('frenns_id', $usernumber)->where('type', '!=', 'expense')->orderBy('last_updated', 'desc')->limit(1)->get();               
                ...
                $idArray = array();              
                $getTodayInsertedInvoice = DB::table('syncinvoice')->where('frenns_id', $usernumber)->Where('last_updated', 'like', '%' . $lastUpdatedTime . '%')->get();              
                ...
                foreach ($invoices as $key2 => $value2) {
                    ...
                    if ($invoice_prefix == 'INV') {
                        ...
                        $invoiceUpdateId 	= 	$accountSystem.'-'.$usernumber . '-' . $value2->invoiceNumber;
                        $invoiceItemUpdateId = 	$accountSystem.'-'.$usernumber . '-' . $value2->type . $value2->invoiceNumber;	
                        ...
                        if ($createdTimeInvoice > $lastUpdatedTime) {
                            $existId = in_array($invoiceIdd, $idArray);
                            if (!empty($existId)) {
                                ...
                                $addInvoice[$key2]['updateId'] = $invoiceItemUpdateId;
                ...
                foreach ($purchaseinvoices as $key2 => $value2) {
                    ...
                    if ($invoice_prefix == 'PUR') {
                        ...
                        $updatedTimeInvoice = $value2->dateModified;
                        $invoiceId = $value2->type . $value2->invoiceNumber;
                        $updateId = $usernumber . '-' . $value2->type . $value2->invoiceNumber;
                        ...
                        if ($createdTimeInvoice >= $lastUpdatedDate) {
                            ...
                            if (!empty($existId)) {
                                ...
                                if (isset($value2->items)) {
                                    ...
                                    $invoiceItemData = array();
                                    $itemLine = $itemKey + 1;                                    
                                    $updateId = $accountSystem.'-'.$usernumber . '-' . $value2->type . $value2->invoiceNumber;
                                    foreach ($value2->items as $key => $invoiceLine) {
                                        $invoiceItemData[$key . $itemKey]['frenns_id'] = $usernumber;
                                        $invoiceItemData[$key . $itemKey]['invoice_number'] = isset($value2->invoiceNumber) ? $value2->invoiceNumber : '';
                                        $invoiceItemData[$key . $itemKey]['line_number'] = $itemLine;
                                        ...
                                        $invoiceItemData[$key . $itemKey]['amount_total'] = $invoiceLine->unitPrice * $invoiceLine->quantity + $value2->vat;
                                        $invoiceItemData[$key . $itemKey]['invoice_id'] = $value2->type . $value2->invoiceNumber;
                                        $invoiceItemData[$key . $itemKey]['updateId'] = $updateId;
                ...
                $getLastUpdatedContact = DB::table('syncsupplier')->select('last_update')->where('frenns_id', $usernumber)->orderBy('last_update', 'desc')->limit(1)->get();
                ...
                $localContacts = DB::table('syncsupplier')->select('updateId')->where('frenns_id', $usernumber)->orderBy('last_update', 'desc')->limit(1)->get();
                ...
                foreach ($entities as $key => $value1) {
                    ...
                    //echo $value1->id . '---' . $value1->date_modified . '====' . $ContactlastUpdatedTime . '<br>';

                    $updateId = $accountSystem.'-'.$usernumber . '-' . $value1->id;
                    ...
                    } else if ($value1->date_modified > $ContactlastUpdatedTime && $ContactlastUpdatedTime != '' && $type != '') {
            ...
            echo "All informations saved successfully!!";
        }
	}
    ...
    public function freshbookData() {      
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();				
				$this->getInnerFreshBookData($getdetail,$accounting_system);
			}
			else
			{
	...
	public function getInnerFreshBookData($getdetail,$accounting_system)
	{
        ...
        if (!empty($getdetail)) {
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                ...
                $accoutSystem 		= $getdetail[$i]->accounting_system;
                ...
                //print_r($responseToken); echo '<br>'.$usernumber; die;
                ...
                // max time for supplier/customer
                $ContactlastUpdatedTime = '';               
                $getLastUpdatedContact = DB::table('syncsupplier')->select('syncsupplier.last_update')->where('frenns_id', $usernumber)->orderBy('last_update', 'desc')->limit(1)->get();
                ...
                $idArray = array();               
                $getTodayInsertedContact = DB::table('syncsupplier')->select('syncsupplier.updateId')->where('last_update', 'like', $ContactlastUpdatedDate)->get();                
                ...
                foreach ($clientList['response'] as $valueResponse) {
                    foreach ($valueResponse['clients'] as $key => $value1) {
                        ...
                        $updateId 		 		= $accoutSystem.'-'.$usernumber . '-' . $value1['id'];
                        ...
                        if ($createdTimeSupplier >= $ContactlastUpdatedTime) {
                            ...
                            //echo 'update';
                            $updateClientsUpdateId = $accoutSystem.'-'.$usernumber . '-' . $value1['id'];
                            $updateClients[$key]['frenns_id'] = $usernumber;
                ...
                $getLastUpdatedInvoice = DB::table('syncinvoice')->select('last_updated')->where('frenns_id', $usernumber)->get();
                ...
                $getTodayInsertedContact = DB::table('syncsupplier')->select('updateId')->where('frenns_id', $usernumber)->where('last_update', 'like', $ContactlastUpdatedDate)->get();
                ...
                // max time for invoice
                $intype = '1';                
                $getLastUpdatedInvoice = DB::table('syncinvoice')->select('last_updated')->where('frenns_id', $usernumber)->where('type', '!=', 'expense')->orderBy('last_updated', 'desc')->limit(1)->get();               
                if (count($getLastUpdatedInvoice) > 0) {
                    $lastUpdatedTime = $getLastUpdatedInvoice[0]->last_updated;
                    $lastUpdatedDate = date("Y-m-d", strtotime($lastUpdatedTime));
                } else {
                ...
                foreach ($InvoiceList['response'] as $valueResponse) {
                    foreach ($valueResponse['invoices'] as $key2 => $value2) {
                        ...
                        $updateId 			= $accoutSystem.'-'.$usernumber . '-' . $value2['invoiceid'];
                        ...
                        if ($createdTimeInvoice >= $lastUpdatedTime) {
                            ...
                            if (!empty($existId1) || empty($idArray1)) {
                                ...
                                $addInvDetailList[$key2]['type'] = 'invoice';
                                ...
                                foreach ($value2['lines'] as $key3 => $value3) {
                                    ...
                                    $prodDetailList[$kk . $key3]['amount_total'] = $value3['amount']['amount'];
                                    $prodDetailList[$kk . $key3]['line_number'] = $lines;
                                    $prodDetailList[$kk . $key3]['updateId'] = $updateId;
                            ...
                            if ($createdTimeInvoice > $lastUpdatedTime) {
                                ...
                                $updateInvDetailList[$key2]['type'] = 'invoice';
                    ...
                    foreach ($expenseInffo['response'] as $valueResponse) {
                        foreach ($valueResponse['expenses'] as $key2 => $value2) {
                            ...
                            $expIDss = $value2['id'];
                            $updateId = $accoutSystem.'-'.$usernumber . '-' . $value2['expenseid'];
    ...
    public function kashflowData() {              
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();			
				$this->getInnerKashFlowData($getdetail,$accounting_system);
			}
			else
			{				
				$dvd 	= ($countDet / 10);				
	...
	public function getInnerKashFlowData($getdetail,$type)
	{
		if (!empty($getdetail))
		{			
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                ...
                foreach ($contactTypes as $contact) {                    
                    $getLastUpdatedContact = DB::table('syncsupplier')->where('frenns_id', $usernumber)->get();                   
                    if (count($getLastUpdatedContact) > 0) {
                        $ContactlastUpdatedTime = $getLastUpdatedContact[0]->last_update;
                    } else {
                        $ContactlastUpdatedTime = '';
                    ...
                    if ($status == 'OK') {
                        ...
                        $idArray = array();                       
                        $getTodayInsertedContact = DB::table('syncsupplier')->where('frenns_id', $usernumber)->get();                      
                        if (!empty($getTodayInsertedContact)) {
                            foreach ($getTodayInsertedContact as $dbData) {
                                $exp = explode("-", $dbData->updateId);
                                $idArray[] = $exp[1];
                        ...
                        foreach ($response->$outputArray1 as $key => $value) {
                            ...
                            if ($totalRecord == 1) {
								...
								$updateId				= $accountSystem.'-'.$usernumber . '-' . $value->$contactID;
                                //
                                ...
                                foreach ($value as $key1 => $value1) {
                                    ...
                                    $updateId 	= $accountSystem.'-'.$usernumber . '-' . $value1->$contactID;
                                    if ($createdTimeSupplier >= $ContactlastUpdatedTime) {
                                        ...
                                        if (!empty($updateSupplierDetailList)) {   
                                            if (!empty($updateSupplierDetailList['contact']['Supplier'])) {                                                
                                                $updateInvoiceItems = DB::table('syncsupplier')->where('updateId', $updateId)->update($updateSupplierDetailList['Supplier']['Supplier'][$j]);
                                            }
                                            if (!empty($updateSupplierDetailList['contact']['Customer'])) {                                               
                                                $updateInvoiceItems = DB::table('syncsupplier')->where('updateId', $updateId)->update($updateSupplierDetailList['contact']['Customer'][$j]);
                                            }
                ...
                if ($status1 == 'OK') {
                    ...
                    $deleteInvoiceByUser = DB::table('syncinvoice')->where('frenns_id', $usernumber)->delete();
                    foreach ($response1->GetInvoices_RecentResult as $key2 => $valueInv) {
                        $updateIdgetInv = $accountSystem.'-'.$usernumber . '-' . $valueInv->InvoiceDBID;
                        ...
                        if ($totalRecord == 1) {
                            ...
                            $addInvDetailList[$j]['pay_date'] = $valueInv->DueDate;
                            $addInvDetailList[$j]['due_date'] = $valueInv->DueDate;
                            $addInvDetailList[$j]['updateId'] = $updateIdgetInv;
                            ...
                            foreach ($response5->GetInvoiceByIDResult->Lines as $kkkk => $value) {
                                ...
                                $updateId 	= $accountSystem.'-'.$usernumber . '-' . $valueInv->InvoiceDBID;
                                if ($totalLine > 1) {
                                    ...
                                    foreach ($value as $keyss => $value1) {
                                        ...
                                        $prodDetailList[$kk . $keyss]['invoice_number'] = $value1->enc_value->Sort;
                                        if (!empty($value1->enc_value->ProductID)) {
                                    ...
                                    $prodDetailList[$kkk . $j]['invoice_number'] = $value->enc_value->Sort;
                                    if (!empty($value->enc_value->ProductID)) {
                            ...
                            foreach ($valueInv as $key1 => $valueInvArr) {
								$updateId = $accountSystem.'-'.$usernumber . '-' . $valueInvArr->InvoiceDBID;
                                ...
                                $addInvDetailList[$j]['paid'] = $paid;
                                //$addInvDetailList[$key2]['pay_date'] = $valueInv->InvoiceDate;
                                ...
                                foreach ($response6->GetInvoiceByIDResult->Lines as $keysss => $value) {
                                    ...
                                    if ($totalLine > 1) {
                                        ...
                                        foreach ($value as $keyy => $value1) {
                                            ...
                                            $prodDetailList[$keyy . $j]['invoice_number'] = $value1->enc_value->Sort;
                                            if (!empty($value1->enc_value->ProductID)) {
                                        ...
                                        $prodDetailList[$kk1 . $j]['invoice_number'] = $value->enc_value->Sort;
                                        if (!empty($value->enc_value->ProductID)) {
                ...
                if ($status21 == 'OK') {
                    ...
                    $getAddedPlData = DB::table('syncreport_pl')->where('frenns_id', $usernumber)->get();
                    //print_r($getAddedPlData);   die;                    
                    if (count($getAddedPlData) > 0) {
                        $updateData['pl_data'] = $response21;
                        $updatePLData = DB::table('syncreport_pl')->where('frenns_id', $usernumber)->update($updateData);
                    } else {
            ...
            echo 'All information store successfully!!';
        }
    ...
    public function xeroData() {
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();			
				$this->getInnerXeroData($getdetail,$accounting_system);
			}
			else
			{				
				$dvd 	= ($countDet / 10);				
	...
	public function getInnerXeroData($getdetail,$accounting_system)
	{
		#Fetch all quickbook user details
        require_once(app_path('third_party/xerophp/') . 'public.php');
		// echo "<pre>";
		...
		if (!empty($getdetail)) {
			...
			//	die;
                ...
                if ($XeroOAuth->response['code'] == 200) {
                    $getdetail = DB::table('syncreport_pl')->select('syncreport_pl.frenns_id')->where('frenns_id', $uniqueId)->get();
                    ...
                    //redirect(site_url() . 'redirectToXero?authenticate=1');
                }
                ...
                if ($XeroOAuth->response['code'] == 200) {
                    ...
                    $localContacts = DB::table('syncsupplier')->select('syncsupplier.contactId')->where('frenns_id', $uniqueId)->get();
                    ...
                    foreach ($contacts['Contacts']['Contact'] as $key => $customer) {
                        ...
                        $updateId = $accountSystem.'-'.$uniqueId . '-' . $customer['ContactID'];
                        ...
                        if (!in_array($customer['ContactID'], $localContactsArr) && $contactType != '') {
                            ...
                            #Save Customer Data
                            if (!empty($customerData)) {
                                //echo "Add Array <pre>"; print_r($customerData);
                                //$this->apidetail->addSuppliers($customerData);
                                $insertPLData = DB::table('syncsupplier')->insert($customerData);
                            }
                            ...
                            $updateCustomerData[$key]['Email'] = isset($customer['PrimaryEmailAddr']['Address']) ? $customer['PrimaryEmailAddr']['Address'] : '';
                            $updateCustomerData[$key]['collection_date'] = date('Y-m-d');
                            $updateCustomerData[$key]['last_update'] = date('Y-m-d');
                            $updateCustomerData[$key]['contactId'] = $contactId;
                            $updateCustomerData[$key]['updateId'] = $updateId;

                            #Update Customer Data
                            if (!empty($updateCustomerData)) {
                                //echo "Update Array <pre>"; print_r($updateCustomerData);
                                //$this->apidetail->updateSuppliers($updateCustomerData, 'updateId');
                                $updatePLData = DB::table('syncsupplier')->where('updateId', $updateId)->update($updateCustomerData);
                            }
                        } else {
                            ## No Action needed.
                        }
                ...
                if ($XeroOAuth->response['code'] == 200) {
                    ...
                    $localInvoices = DB::table('syncinvoice')->select('syncinvoice.invoiceId')->where('frenns_id', $uniqueId)->get();
                    ...
                    //echo "#@#<pre>"; print_r($localIvoicesArr); die;

                    foreach ($invoices['Invoices']['Invoice'] as $key => $invoices) {
                        //echo '###<pre>';print_r($invoices); die(' Single Invoices');
                        ...
                        if ($XeroOAuth->response['code'] == 200) {
                            ...
                            //echo '###<pre>';print_r($invoice); //die(' Single Invoices');
                            if (!in_array($invoice['InvoiceID'], $localIvoicesArr)) {
                                $invoiceUpdateId = $accountSystem.'-'.$uniqueId . '-' . $invoice['InvoiceID'];
                                $invoiceData[$key]['frenns_id'] = $uniqueId;
                                $invoiceData[$key]['company_account_number'] = '';
                                $invoiceData[$key]['collection_date'] = date('Y-m-d');
                                $invoiceData[$key]['last_updated'] = date('Y-m-d');
                                ...
                                //$invoiceData[$key]['issue_date'] = isset($invoice['DateString']) ? $invoice['DateString'] : '';
                                //$invoiceData[$key]['due_date'] = isset($invoice['DueDateString']) ? $invoice['DueDateString'] : '';
                                $invoiceData[$key]['payment_terms'] = '';
                                $invoiceData[$key]['payment_method'] = '';
                                $invoiceData[$key]['delivery_date'] = '';
                                ...
                                $invoiceData[$key]['paid'] = '';
                                $invoiceData[$key]['pay_date'] = '';
                                $invoiceData[$key]['updateId'] = $invoiceUpdateId;
                                ...
                                if (isset($invoice['LineItems'])) {
                                    ...
                                    foreach ($invoice['LineItems'] as $itemKey => $invoiceLine) {
                                        //echo '###<pre>';print_r($invoiceLine); die(' Single Invoices');
                                        if (isset($invoiceLine['LineItemID'])) {
                                            $invoiceItemUpdateId = $accountSystem.'-'.$uniqueId . '-' . $invoices['InvoiceID'] . '-' . $invoiceLine['LineItemID'];
                                            $invoiceItemData[$key . $itemKey]['frenns_id'] = $uniqueId;
                                ...
                                $updateInvoiceData[$key]['collection_date'] = date('Y-m-d');
                                $updateInvoiceData[$key]['last_updated'] = date('Y-m-d');
                                ...
                                $updateInvoiceData[$key]['issue_date'] = isset($invoice['DateString']) ? $invoice['DateString'] : '';
                                $updateInvoiceData[$key]['due_date'] = isset($invoice['DueDateString']) ? $invoice['DueDateString'] : '';
                                $updateInvoiceData[$key]['payment_terms'] = '';
                                $updateInvoiceData[$key]['payment_method'] = '';
                                $updateInvoiceData[$key]['delivery_date'] = '';
                                ...
                                $updateInvoiceData[$key]['paid'] = '';
                                $updateInvoiceData[$key]['pay_date'] = '';
                                $updateInvoiceData[$key]['updateId'] = $invoiceUpdateId;

                                #Update Xero Invoices                                
                                //$this->apidetail->updateInvoice($updateInvoiceData, 'quickbookId');
                                DB::table('syncinvoice')->where('updateId', $invoiceUpdateId)->update($updateInvoiceData[$key]);
                                #Upsate invoice item data
                                if (isset($invoice['Line'])) {
                                    $itemKey = 0;
                                    foreach ($invoice['Line'] as $invoiceLine) {
                                        if (isset($invoiceLine['Id'])) {
                                            $invoiceItemUpdateId = $uniqueId . '-' . $invoices['InvoiceID'] . '-' . $invoiceLine['Id'];
                                            $UpdateInvoiceItemData[$key . $itemKey]['frenns_id'] = $uniqueId;
                                            ...
                                            //$this->apidetail->updateInvoiceItems($UpdateInvoiceItemData, 'quickbookItemId');
    ...
    public function sageOneRecords() 
	{
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();			
				$this->getInnerSageOneRecords($getdetail,$accounting_system);
			}
			else
			{				
				$dvd 	= ($countDet / 10);				
	...
	public function getInnerSageOneRecords($getdetail,$accounting_system)
	{
		if (!empty($getdetail)) {			
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                ...
                $getLastUpdatedLedgerEntriesFirst = DB::table('syncnominal')->select('last_updated')->where('frenns_id', $usernumber)->orderBy('last_updated', 'desc')->limit(1)->get();
                $getLastUpdatedLedgerEntriesSec = DB::table('syncledger_transaction')->select('last_updated')->where('frenns_id', $usernumber)->orderBy('last_updated', 'desc')->limit(1)->get();
...
//              invoice and item
                foreach ($invTypes as $typ) {
                    ...
                    $response = Helper::callApi($base_endpoint, $endpoint, $signing_secret, $token, $sageone_guid, $this->apim_subscription_key);
                    ...
                    foreach ($invoiceProducts as $key2 => $value2) {
                        ...
                        foreach ($value2->invoice_lines as $kk => $v) {
                            ...
                            $prodDetailList[$kk]['updateId'] = $updateId;
                ...
                foreach ($mainItem as $key => $valueItem) {
					...
					$updateIdmainItem 	 = $accountSystem.'-'.$usernumber . '-' . $SupplierContactDetail->id;
                    if ($createdTimeSupplier > $ContactlastUpdatedTime) {
                        ...
                        $addSupplierDetailList[$key]['updateId'] = $updateIdmainItem;
                    ...
                    if ($updatedTimeSupplier > $ContactlastUpdatedTime) {
                        ...
                        $updateSupplierDetailList[$key]['updateId'] = $updateIdmainItem;
                ...
                foreach ($mainledgerEntries as $keyy => $ledgervalueItem) {
					...
					$updateIdmainledger	 = $accountSystem.'-'.$usernumber . '-' . $ledgervalueItem->id;
                    ...
                    if ($nominalBal < '4000') {
                        if ($createdTime > $lastUpdateTime2) {
                            ...
                            $addSingleLedgerList[$keyy]['updateId'] = $updateIdmainledger;
                        ...
                        if ($updatedTime > $lastUpdateTime2) {
                            ...
                            $updateSingleLedgerList[$keyy]['updateId'] = $updateIdmainledger;
                    ...
                    if ($nominalBal > '3999') {
                        ...
                        if ($createdTime > $lastUpdateTime2) {
                            ...
                            $addNominalLedger[$keyy]['updateId'] = $updateIdmainledger;
                            #add nominal ledger 
                        ...
                        if ($updatedTime > $lastUpdateTime2) {
                            ...
                            $updateNominalLedger[$keyy]['updateId'] = $updateIdmainledger;
    ...
    public function exactData() {
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();
				//echo "<pre>"; print_r($getdetail); echo "<pre>"; die;
				$this->getInnerExactData($getdetail,$accounting_system);
			}
			else
			{
    ...
    #
	public function getInnerExactData($getdetail,$accounting_system)
	{		
		if (!empty($getdetail)) {
            $response = array();
            $addSupplierDetailList = array();
            ...
            for ($i = 0; $i < count($getdetail); $i++) {				
                ...
                try {
                    ...
                    $intype = '1';                                      
                    $getLastUpdatedInvoice = DB::table('syncinvoice')->where('frenns_id', $usernumber)->where('type', '!=', 'expense')->get();
                    if (count($getLastUpdatedInvoice) > 0) 
					{
                        $lastUpdatedTime = $getLastUpdatedInvoice[0]->last_updated;
                        $lastUpdatedDate = date("Y-m-d", strtotime($lastUpdatedTime));
                    ...
                    $dbInvoice = DB::table('syncinvoice')->select('syncinvoice.invoiceId')->where('frenns_id', $usernumber)->get();
                    ...
                    foreach ($invoiceRes as $j => $valueInv) {
                        ...
                        $updateId	 	= $accoutSystem.'-'.$usernumber . '-' . $valueInv->InvoiceID;
                        if (!in_array($valueInv->InvoiceID, $dbInvoiceArr)) {
                            ...
                            //$addInvDetailList[$j]['paid'] = $paid;
                            $addInvDetailList[$j]['pay_date'] = $valueInv->InvoiceDate;
                            ...
                            //$addInvDetailList[$j]['paid'] = $paid;
                            $updateInvoice[$j]['pay_date'] = $valueInv->InvoiceDate;
                    ...
                    foreach ($InvoiceLines as $key => $invoiceLine) {
						...
						$updateId = $accoutSystem.'-'.$usernumber . '-' . $invoiceLine->InvoiceID;
                        ...
                        $invoiceItem[$key]['updateId'] = $updateId;
                    ...
                    //get cust/suplier from db
                    //$localContacts = $this->apidetail->getCustomerSuppliersIds($usernumber);
                    $localContacts = DB::table('syncsupplier')->select('contactId')->where('frenns_id', $usernumber)->get();
                    $localContactsArr = array();
                    if (!empty($localContacts)) {
                        foreach ($localContacts as $key => $localContact) {
                    ...
                    foreach ($result as $key => $value1) {
                        ...
                        $updateId = $accounting_system.'-'.$usernumber . '-' . $value1->ID;
    ...
    public function exactDataUK() {
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();
				//echo "<pre>"; print_r($getdetail); echo "<pre>"; die;
				$this->getInnerExactDataUK($getdetail,$accounting_system);
			}
			else
			{
	...
	public function getInnerExactDataUK($getdetail,$accounting_system)
	{		
        if (!empty($getdetail)) {
			//echo "<pre>"; print_r($getdetail); echo "</pre>"; die;
            $response = array();
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                ...
                try {
                    ...
                    foreach ($InvoiceLines as $key => $invoiceLine) {
                        ...
                        $invoiceItem[$key]['frenns_id'] = $usernumber;
                        //$invoiceItem[$key]['invoice_number'] = isset($invoiceLine->invoiceNumber) ? $invoiceLine->invoiceNumber : '';
						$updateInvoiceItemId	=	$accoutSystem.'-'.$usernumber . '-' . $invoiceLine->InvoiceID;
                        $invoiceItem[$key]['line_number'] = $invoiceLine->LineNumber;
                        $invoiceItem[$key]['product_code'] = $invoiceLine->ItemCode;
                        $invoiceItem[$key]['description'] = $invoiceLine->ItemDescription;
                        $invoiceItem[$key]['qty'] = $invoiceLine->Quantity;
                        ...
                        $invoiceItem[$key]['amount_total'] = $invoiceLine->NetPrice * $invoiceLine->Quantity + $invoiceLine->VATAmountFC;
                        $invoiceItem[$key]['invoice_id'] = $invoiceLine->InvoiceID;
                        $invoiceItem[$key]['updateId'] = $updateInvoiceItemId;
                        if (!empty($invoiceItem)) {
                            //$addInvoiceItem = $this->apidetail->addInvoiceItem($invoiceItem, '');
                            $addInvoiceItem = DB::table('syncinvoice_item')->insert($invoiceItem[$key]);
                        }
    ...
    public function reeleezeeData() {     
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();
				//echo "<pre>"; print_r($getdetail); echo "<pre>"; die;
				$this->getInnerReeleezeeData($getdetail,$accounting_system);
			}
			else
			{
	...
	public function getInnerReeleezeeData($getdetail,$accounting_system)
	{
		if (!empty($getdetail)) {			
            for ($i = 0; $i < count($getdetail); $i++) {
                $usernumber 	= $getdetail[$i]->usernumber;
                $username 		= $getdetail[$i]->UserName;
                $password 		= $getdetail[$i]->Password;
                $accoutSystem 	= $getdetail[$i]->accounting_system;
                // sale invoice
                $getLastUpdatedInvoice = DB::table('syncinvoice')->select('last_updated')->where('frenns_id', $usernumber)->where('type', 'Sale Invoice')->orderBy('last_updated', 'desc')->limit(1)->get();
                //print_r($getLastUpdatedInvoice); die('==');
                if (count($getLastUpdatedInvoice) > 0) {
                    $lastUpdatedTime = $getLastUpdatedInvoice[0]->last_updated;
                    $lastUpdatedDate = date("Y-m-d", strtotime($lastUpdatedTime));
                ...
                $idArray = array();
                $getTodayInsertedInvoice = DB::table('syncinvoice')->select('invoiceId')->where('frenns_id', $usernumber)->where('type', 'Sale Invoice')->where('last_updated', 'like', $lastUpdatedTime)->get();
                ...
                if (!empty($result)) {
                    ...
                    foreach ($result1->value as $key2 => $value2) {
                        $updateId 	= $accoutSystem.'-'.$usernumber . '-' . $value2->id;
                        $invoiceID 	= $value2->id;
                        if ($value2->TotalNetAmount != 0) {
                            ...
                            if (!empty($result2)) {
                                ...
                                foreach ($result3->value as $key => $invoiceLine) {
                                    $invoiceItemUpdateId = $accoutSystem.'-'.$usernumber . '-' . $value2->id . '-' . $invoiceLine->id;
                ...
                $getLastUpdatedPInvoice = DB::table('syncinvoice')->select('last_updated')->where('frenns_id', $usernumber)->where('type', 'Purchase Invoice')->orderBy('last_updated', 'desc')->limit(1)->get();
                //print_r($getLastUpdatedInvoice); die('==');
                ...
                $idArray2 = array();
                $getTodayInsertedPInvoice = DB::table('syncinvoice')->select('invoiceId')->where('frenns_id', $usernumber)->where('type', 'Purchase Invoice')->where('last_updated', 'like', $lastUpdatedTime1)->get();
                ...
                $localContacts = DB::table('syncsupplier')->select('contactId')->where('frenns_id', $usernumber)->where('type', 'Customer')->get();
                $localContactsArr = array();
                if (!empty($localContacts)) {
                    foreach ($localContacts as $key => $localContact) {
                        $localContactsArr[] = $localContact->contactId;
    ...
    function freeagentData() {		
		...
		if ($userNumber == '') {            
			...
			{				
				$getdetail = DB::table('synccredential')->where('accounting_system', $accounting_system)->get();
				//echo "<pre>"; print_r($getdetail); echo "<pre>"; die;
				$this->getInnerFreeAgentData($getdetail,$accounting_system,$clinetId,$clientSecret);
			}
			else
			{				
				$dvd 	= ($countDet / 10);				
	...
	public function getInnerFreeAgentData($getdetail,$accounting_system,$clinetId,$clientSecret)
	{	
		if (!empty($getdetail)) {
            $response = array();
            $addSupplierDetailList = array();
            ...
            for ($i = 0; $i < count($getdetail); $i++) {
                $usernumber = $getdetail[$i]->usernumber;
                $code = $getdetail[$i]->code;
                $access_token_db = $getdetail[$i]->access_token;
                $refresh_token_db = $getdetail[$i]->refresh_token;
                $uniqueId = $getdetail[$i]->accounting_system.'-'.$getdetail[$i]->usernumber;
                ...
                $getLastUpdatedContact = DB::table('syncsupplier')->select('last_update')->where('frenns_id', $uniqueId)->orderBy('last_update', 'desc')->limit(1)->get();
                 if (count($getLastUpdatedContact) > 0) {
                    $ContactlastUpdatedTime = $getLastUpdatedContact[0]->last_update;
                } else {
                    $ContactlastUpdatedTime = '';
                ...
                $idArray = array();
                $getTodayInsertedContact = DB::table('syncsupplier')->select('updateId')->where('frenns_id', $uniqueId)->where('last_update', 'like', $ContactlastUpdatedTime)->get();
                //print_r($getTodayInsertedContact); die;
                if (!empty($getTodayInsertedContact)) {
                    foreach ($getTodayInsertedContact as $dbData) {
                        $exp = explode("-", $dbData->updateId);
                ...
                //invoice  
                // max time for invoice
                $getLastUpdatedInvoice = DB::table('syncinvoice')->select('last_updated')->where('frenns_id', $uniqueId)->orderBy('last_updated', 'desc')->limit(1)->get();
                if (count($getLastUpdatedInvoice) > 0) {
                ...
                $idArray1 = array();
                $getTodayInsertedInvoice = DB::table('syncsupplier')->select('updateId')->where('frenns_id', $uniqueId)->where('last_update', 'like', $invoicelastUpdatedTime)->get();
                //print_r($getTodayInsertedContact); die;
                ...
                if (!empty($invoices)) {
                    foreach ($invoices['result']['invoices'] as $key2 => $value2) {
                        ...
                        if ($created_at1 >= $invoicelastUpdatedTime || empty($invoicelastUpdatedTime)) {
                            ...
                            if (!empty($existId)) {
                                ...
                                if (isset($invoice['LineItems']) && !empty($invoice['LineItems'])) {
                                    ...
                                    foreach ($invoice['LineItems'] as $itemKey => $invoiceLine) {
                                        ...
                                        if (isset($itemId)) {
                                            $updateItemUpdateId = $uniqueId . '-' . $invoiceId . '-' . $itemId;
                                            ...
                                            $updateItemData[$key2 . $itemKey]['invoice_id'] = $invoiceId . '-' . $itemId;
                                ...
                                if (isset($invoice['LineItems']) && !empty($invoice['LineItems'])) {
                                    ...
                                    foreach ($invoice['LineItems'] as $itemKey => $invoiceLine) {
                                        $itemId = substr($invoiceLine['url'], strrpos($invoiceLine['url'], '/') + 1);                                        
                                        if (isset($itemId)) {
                                            $invoiceItemUpdateId = $uniqueId . '-' . $invoiceId . '-' . $itemId;
                                            $invoiceItemData[$key2 . $itemKey]['frenns_id'] = $usernumber;
                                            //$invoiceItemData[$key2 . $itemKey]['invoice_number'] = isset($invoice['InvoiceNumber']) ? $invoice['InvoiceNumber'] : '';
                                            $invoiceItemData[$key2 . $itemKey]['line_number'] = $itemId;
                                            //$invoiceItemData[$key2 . $itemKey]['product_code'] = isset($invoiceLine['ItemCode']) ? $invoiceLine['ItemCode'] : '';
                                            $invoiceItemData[$key2 . $itemKey]['description'] = isset($invoiceLine['description']) ? $invoiceLine['description'] : '';
                                            $invoiceItemData[$key2 . $itemKey]['qty'] = isset($invoiceLine['quantity']) ? $invoiceLine['quantity'] : '';
                                            $invoiceItemData[$key2 . $itemKey]['rate'] = isset($invoiceLine['price']) ? $invoiceLine['price'] : '';
                                            $invoiceItemData[$key2 . $itemKey]['amount_total'] = $invoiceLine['price'] * $invoiceLine['quantity'];
                                            $invoiceItemData[$key2 . $itemKey]['invoice_id'] = $invoiceId . '-' . $itemId;
                                            $invoiceItemData[$key2 . $itemKey]['updateId'] = $invoiceItemUpdateId;

                                            #Save FreeAgent Invoices items 
                                            if (!empty($invoiceItemData)) {
                                                //$this->apidetail->addInvoiceItem($invoiceItemData);                                              
                                                DB::table('syncinvoice_item')->insert($invoiceItemData[$key2 . $itemKey]);
                                            }
                                            $itemKey++;
                                        }
                                    }
                            ...
                            $updateInvoice[$key2]['type'] = 'Sales Invoice';
                            $updateInvoice[$key2]['payment_terms'] = $value2->payment_terms_in_days;
                            $updateInvoice[$key2]['updateId'] = $usernumber . '-' . $invoiceId;
                            ...
                            if (isset($invoice['LineItems']) && !empty($invoice['LineItems'])) {
                                ...
                                foreach ($invoice['LineItems'] as $itemKey => $invoiceLine) {
                                    $itemId = substr($invoiceLine['url'], strrpos($invoiceLine['url'], '/') + 1);
                                    //echo '###<pre>';print_r($invoiceLine); die(' Single Invoice Items');
                                    if (isset($itemId)) {
                                        $updateItemUpdateId = $uniqueId . '-' . $invoiceId . '-' . $itemId;
                                        $updateItemData[$key2 . $itemKey]['frenns_id'] = $usernumber;
                                        //$updateItemData[$key2 . $itemKey]['invoice_number'] = isset($invoice['InvoiceNumber']) ? $invoice['InvoiceNumber'] : '';
                                        ...
                                        if (!empty($updateItemData)) {
                                            //$this->apidetail->addInvoiceItem($invoiceItemData);                                              
                                            DB::table('syncinvoice_item')->where('updateId', $updateItemUpdateId)->update($updateItemData[$key2 . $itemKey]);
                                        }
                                        $itemKey++;
                                    }
                                }
	...
	public function freshbookDataTest1() {
        ...
        $grant_type = 'authorization_code';
        $request_type = 'code';        
        $request_type_val ='0b80f3bb87de891cc30e9b67b267b90c9e3541136568250dc5c5ed75a4ad01a0';        
        $curl = curl_init();
